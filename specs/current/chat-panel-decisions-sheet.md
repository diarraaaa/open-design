...
1. **「跑通了但余额低」那一档刷新后仍会消失。** 那一轮**根本没有 error 事件可挂**
   (它跑通了),消息上不留任何痕迹。要盖住它得给 `messages` 表加顶层字段
   (类型化列,要 `ALTER TABLE` + 改 `listMessages` / `upsertMessage` /
   `normalizeMessage`,约 4–5 个文件 8 处改点)。**没做,等产品要**。
