...
- Preview bridges are negotiated capabilities on a real-URL document, NOT srcDoc injections. `apps/daemon/src/http/preview-runtime-bootstrap.ts` injects one bootstrap into the served HTML, the modules in `preview-runtime-modules.ts` `register()` against it (deck, selection/comment/inspect/draw, palette, edit, tweaks, scroll, snapshot, observability, presentation), and the host enables them per frame through `od:preview:set-capabilities`. `apps/web/src/runtime/preview-runtime-capabilities.ts` maps viewer state to that set, `PreviewRuntimeTransport` replays a mode payload only for capabilities the runtime has acknowledged, and `PreviewSession` promotes a frame only once it has acknowledged the exact set it was asked for. A capability the runtime never acknowledges therefore receives no mode message at all — a missing acknowledgement is why a bridge looks dead, and it is the first thing to check.
- FileViewer's preview stack renders only `PreviewRuntimeTransport` (`preview-runtime-frame-current` / `-standby`); there is no srcDoc preview iframe left in it. `apps/web/src/components/file-viewer-render-mode.ts` now only supplies source-content heuristics: which guards the daemon should inject (sandbox shim, focus, redirect, powered) and whether the host needs the full source before first paint (`hasTweaksTemplate`). None of them routes a feature onto a srcDoc-only transport any more. Do not add a "srcDoc-only bridge" disqualifier; add a capability module instead.
- Historical note, because this line was wrong for long enough to mislead an investigation: bridges used to inject ONLY through srcDoc, and `UrlLoadDecision` used to carry per-feature disqualifiers. That is the pre-convergence product; see `specs/current/html-preview-runtime-convergence.md`.
