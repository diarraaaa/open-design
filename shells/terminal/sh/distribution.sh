...
standalone_launcher_sha=$(sha256_file "$root/seed/standalone-launcher.mjs")
...
printf '{"capabilities":{"contentUpdater":"standalone-v4","sharedInstance":"sidecar-v1","shellUpdater":"unavailable"},"carrierLock":{"file":"carrier.lock","sha256":"%s"},"contracts":{"file":"contract/index.json","sha256":"%s"},"fossil":{"entrypoint":"runtime/fossil.mjs","sha256":"%s"},"releaseDocuments":{"content":{"file":"release/content-metadata.json","sha256":"%s"}},"runtime":{"executable":"%s","name":"node","sha256":"%s","version":"%s"},"runtimeModules":{"file":"runtime/modules.json","sha256":"%s"},"schemaVersion":1,"seed":{"closure":{"file":"seed/closure.mjs","sha256":"%s"},"standaloneLauncher":{"file":"seed/standalone-launcher.mjs","sha256":"%s"}},"shell":{"buildHash":"%s","type":"terminal","version":"%s"},"shellFiles":{"ps1":{"install":{"file":"ps1/install.ps1","sha256":"%s"},"terminal":{"file":"ps1/terminal.ps1","sha256":"%s"}},"sh":{"install":{"file":"sh/install.sh","sha256":"%s"},"terminal":{"file":"sh/terminal.sh","sha256":"%s"}}},"sidecarBootstrap":{"entrypoint":"runtime/sidecar-bootstrap.mjs","sha256":"%s"},"sidecarHost":{"entrypoint":"runtime/sidecar-host.mjs","sha256":"%s"},"standalone":{"entrypoint":"runtime/standalone/index.mjs","sha256":"%s"},"target":"%s","trust":{"file":"trust/keys.json","sha256":"%s"}}\n' \
  "$carrier_lock_sha" "$contract_index_sha" "$fossil_sha" "$content_sha" "$node_executable" "$node_sha" "$node_version" "$runtime_modules_sha" "$closure_sha" "$standalone_launcher_sha" "$shell_build_hash" "$shell_version" "$ps_install_sha" "$ps_terminal_sha" "$sh_install_sha" "$sh_terminal_sha" "$sidecar_bootstrap_sha" "$sidecar_host_sha" "$standalone_sha" "$target" "$trust_sha" > "$root/install-manifest.json"
